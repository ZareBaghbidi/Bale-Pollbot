...
def _check_admin(uid, admins, message, deny_text):
    if uid not in admins:
        message.reply(deny_text)
        return False
    return True


def _handle_payments(uid, text, message, admins):
    if text != "payments":
        return False

    if not _check_admin(uid, admins, message, "دسترسی denied."):
        return True
...
def _handle_user_payments(uid, text, message, admins):
    if not text.startswith("user_payments"):
        return False
...
def _handle_payments_filter(uid, text, message, admins):
    if not text.startswith("payments_filter"):
        return False
...
def _handle_get_money(uid, text, message, pending_actions, user_states, admins):
    if not text.startswith("get_money"):
        return False

    if not _check_admin(uid, admins, message, "شما دسترسی به این دستور را ندارید."):
        return True
    ...
    kb = InlineKeyboard(
        [("✅ تایید و ارسال", f"confirm_pay_{uid}"),
...
def payment_hadnler(uid, text, message, pending_actions, user_states, admins):
    if _handle_payments(uid, text, message, admins):
        return
    if _handle_user_payments(uid, text, message, admins):
        return
    if _handle_payments_filter(uid, text, message, admins):
        return
    if _handle_get_money(uid, text, message, pending_actions, user_states, admins):
        return
