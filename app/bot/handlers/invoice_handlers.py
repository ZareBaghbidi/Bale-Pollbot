...
def _check_admin(uid, admins, message):
...
def _handle_invoices(uid, text, message, admins):
    if text != "invoices":
        return False

    if not _check_admin(uid, admins, message):
        return True
...
def _handle_invoices_filter(uid, text, message, admins):
    if not text.startswith("invoices_filter"):
        return False
...
def _handle_invoices_unpaid(uid, text, message, admins):
...
def _handle_invoice_stats(uid, text, message, admins):
...
def invoice_hadnler(uid, text, message, admins):
    if _handle_invoices(uid, text, message, admins):
        return
    if _handle_invoices_filter(uid, text, message, admins):
        return
    if _handle_invoices_unpaid(uid, text, message, admins):
        return
    if _handle_invoice_stats(uid, text, message, admins):
        return
